...
export const processColor = (currentColor) => {
  const str = currentColor.slice(4, currentColor.length - 1);
  const arr = str.split(',')

  for(let i in arr){
    const color = Number(arr[i] - 40);
    arr[i] = String(color)
  }
  const [uno, dos, tres] = arr;
  const start = currentColor.slice(0, 4);
  const end = currentColor.slice(currentColor.length - 1, currentColor.length)
  const newColor = start + uno + ',' + dos + ',' + tres + end;
